...
def get_time_blocks_available_discharge_data(camel_id, country):

    # Custom function to create a data object, which will work with eWaterCycle
    forcing_for_discharge_lookup = create_forcing_data_HBV_from_nc("1950-01-01T00:00:00Z", "2020-01-01T00:00:00Z", camel_id, country)
...
def get_station_names(country):
    directory =  f"/data/shared/climate-data/camels_africa/{country.lower()}/camel_data"
    station_names = dict()
...
def stations_map(stations_dict, country):
    # Your dictionary: station_name -> id
    station_names = stations_dict
    
    # Folder with shapefiles
    shapefile_dir = f"/data/shared/climate-data/camels_africa/{country.lower()}/camel_data/shapefiles"
...
def create_forcing_data_HBV_from_nc(start_date, end_date, camel_id, country):
    central_path =  f"/data/shared/climate-data/camels_africa/{country.lower()}/camel_data"
    path_to_save = Path.home() / "my_data/workshop_africa" / "forcing_data" / camel_id / "caravan"
