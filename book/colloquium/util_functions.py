...
def Caravan_ERA5_to_local(caravan_forcing, ERA5_forcing):

    start_time = pd.to_datetime(ERA5_forcing.start_time).tz_localize(None)
    end_time = pd.to_datetime(ERA5_forcing.end_time).tz_localize(None)

    def align_time_to_noon(dataarray):
        return dataarray.assign_coords(time=dataarray.time + pd.Timedelta(hours=12))

    # print(ERA5_forcing["pr"])
    my_data_nc_pr = ERA5_forcing["pr"]
    my_data_nc_rsds = ERA5_forcing["rsds"]
    my_data_nc_tas = ERA5_forcing["tas"]
    my_data_nc_potevap = ERA5_forcing["evspsblpot"]
    my_data_nc_discharge = caravan_forcing["Q"]
    # print(ERA5_forcing.start_time)
    # Load NetCDF variables
    P = xr.open_dataset(my_data_nc_pr, engine="netcdf4")['pr']
    P = P[(P.time >= start_time) & (P.time < end_time)]  
    P = abs(P)
    # print(P)
    
    PE = xr.open_dataset(my_data_nc_potevap, engine="netcdf4")['evspsblpot']
    PE = PE[(PE.time >= start_time) & (PE.time < end_time)]  
    
    Q = xr.open_dataset(my_data_nc_discharge, engine="netcdf4")['Q']
    Q = Q[(Q.time >= start_time) & (Q.time < end_time)]
    Q = align_time_to_noon(Q)  #  Shift Q to 12:00
...
def plot_hydrograph_local(I_max, Ce, Su_max, beta, P_max, T_lag, Kf, Ks, Fm, model, forcing):
    Sin = np.array([0,  100,  0,  5,  0])
    Par = np.array([I_max, Ce, Su_max, beta, P_max, T_lag, Kf, Ks, Fm])
    directory_tmp = Path.home() / "temp"
    directory_tmp.mkdir(exist_ok=True, parents=True)
    config_file, _ = model.setup(parameters=Par, initial_storage=Sin, cfg_dir=directory_tmp)
    ...
    while model.time < model.end_time:
        model.update()
        Qm.append(model.get_value("Q")[0])
        time.append(pd.Timestamp(model.time_as_datetime))
    
    # Qm = model(Par, forcing, Sin, hydrograph='FALSE')
    # print(Qm)

    Qm_array = np.array(Qm)
    # Calculate NSE
    Qo = forcing['Q'].values
    Qo = np.array(Qo)
    ind = np.where(Qo >= 0)
    QoAv = np.mean(Qo[ind])
    ErrUp = np.sum((Qo[ind] - Qm_array[ind]) ** 2)
...
def interactive_plot_local(model, forcing, params):
    ...
    sliders = {
        ...
        'Fm': FloatSlider(min=params['Fm']['min'], max=params['Fm']['max'], step=0.0001, value=Fm, readout_format='.3f')
    }
